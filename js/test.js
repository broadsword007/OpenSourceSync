  ...
  onAddAPCButton:function () {
    ...
    bonjour.find({ type: 'OSSActiveHost' }, function (service) {
        ...
        if(service.host != os.hostname()) {
            ...
            //this.onSearchListEntry(service.referer.address, service.host);
            //flag = false
            }          
        }
        else {
    ...
    onSearchListEntry: function (ipAddress, hostname) {
    ...
    attachIOHook: function(){
        console.log("Attaching IOHook")
        hook.on("mousemove", event => {
            console.log("Mouse event generated : ", connectedPCs.length)
